...
use App\Models\ChiefComplaintTable;
use App\Models\FinancialStateTable;
use App\Models\MedicalHistoryTable;
...
class FormService
{
    ...
    public function updateOrCreate(Request $request, $id)
    {
        switch ($request->form_name) {
            case 'physiological_measurement':
                return $this->update_or_create_physiological_measurement($request, $id);
            case 're_physiological_measurement':
                return $this->update_or_create_re_physiological_measurement($request, $id);
            case 'medical_condition':
                return $this->update_or_create_medical_condition($request, $id);
            case 'medication_adherence':
                return $this->update_or_create_medication_adherence($request, $id);
            case 'lubben_social_network_scale':
                return $this->update_or_create_lubben_social_network_scale($request, $id);
            case 'social_background':
                return $this->update_or_create_social_background($request, $id);
            case 'function_mobility':
                return $this->update_or_create_function_mobility($request, $id);
            case 'barthel_index':
                return $this->update_or_create_barthel_index($request, $id);
            case 'geriatric_depression_scale':
                return $this->update_or_create_geriatric_depression_scale($request, $id);
            case 'iadl':
                return $this->update_or_create_iadl($request, $id);
            case 'montreal_cognitive_assessment':
                return $this->update_or_create_montreal_cognitive_assessment($request, $id);
            case 'physical_condition':
                return $this->update_or_create_physical_condition($request, $id);
            case 'assessment_case_status':
                return $this->update_or_create_assessment_case_status($request, $id);
            case 'qualtrics':
                return $this->update_or_create_qualtrics($request, $id);
            case 'social_worker':
                return $this->update_or_create_social_worker($request, $id);
            default:
                return 'Invalid form name';
        }
    }

    public function update_or_create_physiological_measurement(Request $request, $id)
    {
        $this->validator->validate_physiological_measurement($request);
    ...
    public function update_or_create_re_physiological_measurement(Request $request, $id)
    {
        $this->validator->validate_re_physiological_measurement($request);
    ...
    public function update_or_create_medical_condition(Request $request, $id)
    {
        $this->validator->validate_medical_condition($request, $id);
        ...
        $medical_history_list = array(array());
        ...
        } else if ($obj_length_cm > 0 && $medical_history) {
        ...
        $chief_complaint_list = array(array());
        ...
        } else if ($obj_length_cf > 0 && $chief_complaint) {
        ...
        } else if (!$request->has('time') || !$request->time) {
        } else if (!$request->has('appointment_id') || !$request->appointment_id) {
        } else {
    ...
    public function update_or_create_medication_adherence(Request $request, $id)
    {
        $this->validator->validate_medication_adherence($request);
    ...
    public function update_or_create_lubben_social_network_scale(Request $request, $id)
    {

        $this->validator->validate_lubben_social_network_scale($request);
    ...
    public function update_or_create_social_background(Request $request, $id)
    {
        $this->validator->validate_social_background($request);
        ...
        $financial_state_list = array(array());
        ...
        } else if ($obj_length_cm > 0 && $community_resource) {
        ...
        } else if ($obj_length > 0 && $living_status) {
        ...
        } else if ($obj_length_fs > 0 && $financial_state) {
    ...
    public function update_or_create_function_mobility(Request $request, $id)
    {
        $this->validator->validate_function_mobility($request);
    ...
    public function update_or_create_barthel_index(Request $request, $id)
    {

        $this->validator->validate_barthel_index($request);
    ...
    public function update_or_create_geriatric_depression_scale(Request $request, $id)
    {

        $this->validator->validate_geriatric_depression_scale($request);
    ...
    public function update_or_create_iadl(Request $request, $id)
    {

        $this->validator->validate_iadl($request);
    ...
    public function update_or_create_montreal_cognitive_assessment(Request $request, $id)
    {

        $this->validator->validate_montreal_cognitive_assessment($request);
        ...
        $sosical_worker = SocialWorkerForm::where('assessment_case_id', $id)->first();
        if ($request->category_percentile > 1){
            if($qualtric->timedup_sec < 30 && $sosical_worker->diagnosed_dementia == 0){
                //set cga_type
                $cga_type = 'CGA Health Coaching';
                //update cga_type to elderly service
                $assessment_case = AssessmentCase::where('id', $id)->first();
                $response_status = $this->externalService->change_cga_type($assessment_case->case_id, $cga_type, $request->bearerToken());
                if ($response_status != 200) {
                    error_log('Failed update cga_type with case id ' . $assessment_case->case_id);
                }
            }
        }else if ($request->category_percentile <= 1){
            if($qualtric->timedup_sec >= 30 && $sosical_worker->diagnosed_dementia == 1){
    ...
    public function update_or_create_physical_condition(Request $request, $id)
    {
        $this->validator->validate_physical_condition($request);
        ...
        $pain_site_list = array(array());
        for ($i = 0; $i < $obj_length; $i++) {
            // Pain 
            // $pain_site_list[$i]['is_pain'] = $request->pains[$i]['is_pain'] ? $request->pains[$i]['is_pain'] : null;
    ...
    public function update_or_create_qualtrics(Request $request, $id)
    {
        $this->validator->validate_qualtrics($request);
        $form = QualtricsForm::updateOrCreate(
            ...
            [
                ...
                // 'body_temperature1' => $request->body_temperature1,
                // 'body_temperature2' => $request->body_temperature2,
                // 'sit_upward1' => $request->sit_upward1,
                // 'sit_upward2' => $request->sit_upward2,
                // 'sit_depression1' => $request->sit_depression1,
                // 'sit_depression2' => $request->sit_depression2,
                // 'sta_upward1' => $request->sta_upward1,
                // 'sta_upward2' => $request->sta_upward2,
                // 'sta_depression1' => $request->sta_depression1,
                // 'sta_depression2' => $request->sta_depression2,
                // 'blood_ox1' => $request->blood_ox1,
                // 'blood_ox2' => $request->blood_ox2,
                // 'heartbeat1' => $request->heartbeat1,
                // 'heartbeat2' => $request->heartbeat2,
                // 'blood_glucose1' => $request->blood_glucose1,
                // 'blood_glucose2' => $request->blood_glucose2,
                // 'phy_kardia' => $request->phy_kardia,
                // 'phy_waist' => $request->phy_waist,
        ...
        $hospitalization_list = array(array());
        for ($i = 0; $i < $obj_length; $i++) {
            // Pain 
        ...
        if ($request->timedup_sec < 30){
            if($MCA->category_percentile > 1 && $sosical_worker->diagnosed_dementia == 0){
                //set cga_type
                $cga_type = 'CGA Health Coaching';
                //update cga_type to elderly service
                $assessment_case = AssessmentCase::where('id', $id)->first();
        ...
        }else if($request->timedup_sec >= 30){
            if($MCA->category_percentile <= 1 && $sosical_worker->diagnosed_dementia == 1){
                $cga_type = 'CGA Nurse Program';
                $assessment_case = AssessmentCase::where('id', $id)->first();
                $response_status = $this->externalService->change_cga_type($assessment_case->case_id, $cga_type, $request->bearerToken());
                if ($response_status != 200) {
        ...
        } else if ($obj_length > 0 && $hospitalization) {
        ...
        } else if ($obj_length_wa > 0 && $walk_aid) {
    ...
    public function update_or_create_social_worker(Request $request, $id)
    {
        $this->validator->validate_social_worker($request);
        $form = SocialWorkerForm::updateOrCreate(
            ...
            [
                ...
                // Cognitive
                // 'forget_stuff' => $request->forget_stuff,
                // 'forget_friend' => $request->forget_friend,
                // 'forget_word' => $request->forget_word,
                // 'correct_word' => $request->correct_word,
                // 'bad_memory' => $request->bad_memory,
        ...
        if($request->diagnosed_dementia == 0 ){
        ...
        }else if($request->diagnozed_dementia == 1){
            if($MCA->category_percentile <= 1 && $qualtrics->timedup_sec >= 30){
                $cga_type = 'CGA Nurse Program';
                $assessment_case = AssessmentCase::where('id', $id)->first();
                $response_status = $this->externalService->change_cga_type($assessment_case->case_id, $cga_type, $request->bearerToken());
        ...
        } else if ($obj_length > 0 && $do_referral) {
        ...
        $elder_home_fall_list = array(array());
        ...
        } else if ($obj_length_hf > 0 && $elder_home_fall) {
        ...
        $elder_home_hygiene_list = array(array());
        ...
        } else if ($obj_length_hh > 0 && $elder_home_hygiene) {
        ...
        } else if ($obj_length_hs > 0 && $home_service) {
        ...
        } else if ($obj_length_ls > 0 && $life_support) {
        ...
        } else if ($obj_length_el > 0 && $elder_living) {
    ...
    public function update_or_create_assessment_case_status(Request $request, $id)
    {
